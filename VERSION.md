...
| 383 | `(эта сборка)` | мажорный | **20.0.0** | двадцатая базовая форма: модульная настенная плитка — рельеф считается от глобальной координаты панно, узор через шов не обрывается |
