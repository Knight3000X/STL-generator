...
| 373 | `(эта сборка)` | минорный | **18.37.0** | импорт стал честным: деталь больше не приезжает на боку, геометрия пишется в каждую запись, имя и цвет берутся из метаданных слайсера |
