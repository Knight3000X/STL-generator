...
Такой коммит в истории ровно один: **`2b0b1e9`** — описание червяка в README, дописанное к уже выпущенной
v16.5.4 после того, как её строка была закрыта. Правильно было положить его в саму v16.5.4; исправлять это
задним числом нечем, поэтому он записан здесь. `test_version.js` сверяет реестр с историей репозитория и
знает про это исключение поимённо: следующий такой коммит уронит батарею, а не пройдёт молча, как этот.
...
| 331 | `(эта сборка)` | второстепенная | **18.6.0** | образцы получили зубья по краям — плита собирается из них как пазл |
