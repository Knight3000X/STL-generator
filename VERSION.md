...
| 402 | `(эта сборка)` | минорный | **22.9.0** | номер слота и его цвет приписаны к именам деталей в 3MF: имя читает любой слайсер, поэтому «какой слот каким цветом» видно и без шаблона проекта, — а многоцветный экспорт без шаблона больше не молчит |
