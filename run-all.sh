...
# СВЕРЯЛКА СЛЕПКОВ ПРОВЕРЯЕТ САМА СЕБЯ, И ДЕЛАЕТ ЭТО ПЕРВЫМ ДЕЛОМ. Инструмент, который по любой паре
# отвечает «различий нет», хуже отсутствующего: он выдаёт зелёный свет, ничего не проверив. Так и вышло
# при переносе слепка в репозиторий — имя поля разбиралось нежадным шаблоном, тот съедал по одной букве,
# и сверка молчала на любой паре. Поймал это первый настоящий прогон, а не проверка.
# СТОИТ ЭТО В НАЧАЛЕ, а не в конце рядом с прочими сторожами, и это не вкусовщина: батарея бросает
# работу на первом упавшем файле, до конца не доходит, и поставленная там самопроверка не гонялась бы
# ровно тогда, когда всё и так красное, — то есть когда ошибиться легче всего. Она ни от чего не
# зависит и стоит миллисекунды.
if ! python3 snapshot-diff.py --self-test; then
  echo "САМОПРОВЕРКА СВЕРЯЛКИ СЛЕПКОВ ПРОВАЛЕНА — она отвечала бы «различий нет» на что угодно"; exit 1
fi
LIB="$TMP/lib.js"
...
for t in test_paramrows.js test_wormwheel.js test_vase.js test_keycap.js test_slice.js test_thread.js test_gear.js test_drilljig.js \
          ...
          test_budget.js test_orient.js test_bridge.js test_volume.js test_bellows.js test_labyrinth.js test_oldham.js test_svg_hole.js test_baseplate.js test_import.js test_3mf.js test_palette.js test_arrange.js test_gclamp.js test_material.js test_seal.js test_cardholder.js test_registry.js test_warnings.js; do
...
# конец прогона. Ни один тест такого не увидит: чтобы узнать общее число проверок, надо прогнать всё.
# Поэтому сверка стоит здесь. Файлы обязаны сойтись ТОЧНО; заявленное число проверок — быть не больше
# настоящего (иначе это неправда) и не меньше 85 % от него (иначе это устарело). В README стояло
# «36557 проверок в 114 файлах», когда файлов было 131, а проверок сорок с лишним тысяч.
if [ -f README.md ]; then
  claim="$(grep -oE 'более [0-9 ]+ проверок в [0-9]+ файл' README.md | head -1)"
  cp_="$(printf '%s' "$claim" | grep -oE '[0-9 ]+ проверок' | tr -d ' проверк')"
  cf="$(printf '%s' "$claim" | grep -oE 'в [0-9]+ файл' | grep -oE '[0-9]+')"
  if [ -z "$cp_" ] || [ -z "$cf" ]; then
    echo "README: не нашёл строки «более N проверок в M файле» — сверить нечем"; exit 1
  fi
  lo=$(( total_pass * 85 / 100 ))
  if [ "$cf" -ne "$n" ]; then
    echo "README врёт про файлы: обещано $cf, прогнано $n"; exit 1
  fi
  if [ "$cp_" -gt "$total_pass" ] || [ "$cp_" -lt "$lo" ]; then
    echo "README врёт про проверки: обещано более $cp_, на деле $total_pass (допустимо $lo…$total_pass)"; exit 1
  fi
  echo "README сходится: более $cp_ проверок в $cf файле"
fi
echo "all green"
