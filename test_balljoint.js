...
function coplanarPairs(tris){
  ...
  /* С v25.30.0 сравнивать надо не с объявленным хвостовиком, а с ПОЛНОЙ узкой длиной: гнездо, точёное
     по первому числу, коротко ровно на кусок под телом звена — из-за этого рука и садилась на дно. */
  chk('мелкое гнездо объявлено', WC({artClampH:6}).some(x => /мельче узкой части руки/.test(x)), WC({artClampH:6}));
...
console.log('\n=== концевики руки: хвостовик и держатели ===');
{
  ...
  for (const d of [0, 6, 12]){
    ...
    /* ГДЕ СТОИТ СТУПЕНЬКА — то, ради чего этот замер и заводился. Приложение объявляет хвостовик
       длиной `postLen`, но узкая часть руки ДЛИННЕЕ: хвостовик идёт вверх в тело звена, а барабан
       звена начинается ещё выше. Полная узкая длина названа теперь числом (`postFree`), и проверка
       требует, чтобы это число было ИЗМЕРЕННЫМ, а не заявленным. */
    chk('  полная узкая длина совпала с деталью',
        Math.abs(hStep - e.postFree) < 0.06,
        {'узкая до':+hStep.toFixed(2), спец:+e.postFree.toFixed(2), объявлено:e.postLen});
    chk('  и она длиннее объявленного хвостовика — на кусок под телом звена',
        e.postFree > e.postLen + 1 && Math.abs((e.postFree - e.postLen) - articBodyStartY(a.j,
          Math.max(a.j.ds/2 + 0.4, a.bodyR))) < 1e-9,
        {разница:+(e.postFree - e.postLen).toFixed(2)});
  }
  ...
  /* ГЛУХОЕ ГНЕЗДО СТРУБЦИНЫ ТОЧИТСЯ ПО ПОЛНОЙ УЗКОЙ ДЛИНЕ (v25.30.0). Замысел записан в самом коде:
     «хвостовик упрётся торцом в дно раньше уступа» — значит держать обязан УСТУП, а торец обязан не
     доставать. Пока гнездо считалось по объявленному `postLen`, выходило наоборот. У ПЛИТЫ этой беды
     нет и не было: там гнездо сквозное, и остаток просто выходит снизу — поэтому чинилось одно
     место, а не оба. */
  for (const ov of [{artEndA:'post'}, {artEndA:'post', artPostLen:8}, {artEndA:'post', artPostLen:40}]){
    const cm = artClampSpec(P2(ov)), e = A2(ov).e;
    chk('гнездо струбцины глубже узкой части — уступ сядет ' + JSON.stringify(ov),
        cm.H >= e.postFree + 1 && cm.shallow === false,
        {гнездо:+cm.H.toFixed(2), узкая:+e.postFree.toFixed(2)});
  }
  /* СЛУЧАЙ, ГДЕ СТАРОЕ И НОВОЕ ПРАВИЛО РАСХОДЯТСЯ, и без него подмена «мерить объявленным» проходила
     насквозь: гнездо ГЛУБЖЕ хвостовика, но МЕЛЬЧЕ узкой части. По старому счёту всё хорошо, по делу —
     рука сядет на дно. Это и есть та самая беда, ради которой сборка делалась, поэтому проверка на
     неё обязана быть именной. */
  { const between = {artEndA:'post', artClampH:20};       // 18 < 20 < 25.98
    const cm = artClampSpec(P2(between)), e = A2(between).e;
    chk('гнездо глубже хвостовика, но мельче узкой части — это мелкое гнездо',
        cm.H > e.postLen && cm.H < e.postFree && cm.shallow === true,
        {гнездо:cm.H, хвостовик:e.postLen, узкая:+e.postFree.toFixed(2), shallow:cm.shallow});
    chk('  и об этом сказано', W2(Object.assign({artPart:'clamp'}, between))
          .some(x => /мельче узкой части руки/.test(x))); }
  chk('  мелкое гнездо объявлено, и названы оба числа',
      artClampSpec(P2({artEndA:'post', artClampH:12})).shallow === true &&
      W2({artEndA:'post', artClampH:12, artPart:'clamp'})
        .some(x => /мельче узкой части руки \(26 мм — это хвостовик 18 плюс 8 мм под телом звена\)/.test(x)),
      W2({artEndA:'post', artClampH:12, artPart:'clamp'}));
  chk('  и строка про хвостовик называет ОБА числа',
      W2({artEndA:'post'}).some(x => /ХВОСТОВИК Ø8\.3×18 мм ВМЕСТО шара, а УЗКАЯ ЧАСТЬ руки до уступа — 26 мм/.test(x)),
      W2({artEndA:'post'}));
