...
console.log('\n=== советы по печати отзываются на сильфон ===');
{
  ...
  /* СРЕДНЯЯ ТОЛЩИНА У СИЛЬФОНА С ОБОДАМИ — НЕ ТОЛЩИНА СТЕНКИ, и совет это честно отражает: два
     стоящих обода толщиной пять миллиметров поднимают среднее с 0.8 до 1.05. Ждать здесь «Периметров:
     2» значило бы ждать, что мера соврёт. А вот у сильфона БЕЗ ободов оболочка одна — и там совет
     обязан назвать ровно два прохода. */
  chk('с ободами советуют больше двух периметров — обода тоже деталь',
      rows.some(r => r === 'Периметров: 3'), rows);
  ...
  chk('без ободов — ровно два прохода стенки 0.8 соплом 0.4',
