...
/* Луч поперёк планки на высоте z ЕЁ ПРОФИЛЯ. Профиль лежит в плоскости XZ и там же остаётся: в v18.5.1
   деталь была повёрнута на четверть ради отверстия под саморез, но переставилось при этом всё, а просили
   переставить два отверстия. С v18.23.0 планка стоит как стояла, саморезы ушли в проушины, и профильная
   координата снова читается по Z. Все замеры идут через этот один вход — иначе систему координат
   пришлось бы вписывать в каждую проверку, и какая-нибудь осталась бы мерить пустоту и молча проходить.

   Луч берётся на y = 0, то есть на середине толщины планки; проушины там тоже есть, но они живут за
   торцами и внутрь колонок не заходят. */
const across = (t, z) => solidRuns(t, 0, 0, z);
...
for(const D of [2, 6, 16]) for(const n of [1, 5, 12]) for(const sc of [0, 4]){
  ...
  /* Оболочек столько, сколько тел: планка одна, и с ней две проушины, если саморезы заказаны. Считать
     их «одной» нельзя — они ПЕРЕСЕКАЮТСЯ, а не срастаются, и общих вершин у них нет. Проверяется, что
     каждая замкнута сама по себе: объединение пересекающихся замкнутых оболочек и есть деталь. */
  const parts = meshComponents(t);
  chk('Ø'+D+' ×'+n+' винт '+sc+': тел '+(sc>0?3:1), parts.length === (sc>0 ? 3 : 1), parts.length);
  chk('Ø'+D+' ×'+n+' винт '+sc+': каждое тело замкнуто',
      parts.every(c => manifoldCheck(c,4).watertight), parts.map(c=>manifoldCheck(c,4).openEdges));
}
...
for(const sc of [3, 4, 6]){
  ...
  for(const sg of [-1, 1]){
    ...
    const down = solidRuns(t, 1, 0, sg*s.xScrew), earHalf = s.H/2 - s.earIn;
    chk(nm+': сверху вниз отверстие наружу не выходит', down.length === 2 &&
        Math.abs(down[0][0] + earHalf) < 1e-6 && Math.abs(down[1][1] - earHalf) < 1e-6, down);
  ...
  /* Габарит НЕ повернулся — в этом вся правка. Толщина планки по-прежнему по Y, профиль по Z, а по X
     прибавились две проушины. Если когда-нибудь кто-то снова развернёт деталь целиком, здесь и упрётся. */
  const b = computeBBox(t);
  chk('Ø'+sc+': толщина планки по Y — как было', Math.abs((b.maxY-b.minY) - s.H) < 1e-6, +(b.maxY-b.minY).toFixed(3));
  chk('Ø'+sc+': профиль по Z — как было', Math.abs((b.maxZ-b.minZ) - s.Hb) < 1e-6, +(b.maxZ-b.minZ).toFixed(3));
  chk('Ø'+sc+': длина = планка плюс две проушины', Math.abs((b.maxX-b.minX) - s.totalW) < 1e-6,
      {вышло:+(b.maxX-b.minX).toFixed(3), планка:+s.W.toFixed(2), проушина:+s.earL.toFixed(2)});
}
/* Пазы открыты ВПЕРЁД, к +Z — туда же, куда смотрит лицевая грань планки. Кабель вдавливают спереди, а
   планка тем временем прилегает к стенке задней гранью и держится саморезами сквозь проушины. Луч вдоль
   Z через центр колонки обязан выйти на волю: выше центра кармана материала нет. */
{
  const t = mk({mntCabN:4, mntCabD:6}), s = cableCombSpec(paramState.box);
  const zc = s.Hb/2 - s.rp - s.nl;
  let open = 0;
  for(let k=0;k<s.n;k++){
    const runs = solidRuns(t, 2, s.xAt(k), 0);          // вдоль Z в центре колонки
    if(runs.every(r => r[1] <= zc + 1e-6)) open++;
  }
  chk('все гнёзда открыты вперёд', open === s.n, open+'/'+s.n);
}
