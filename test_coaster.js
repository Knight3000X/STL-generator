...
/* НИ ОДНОГО ЗАДВОЕННОГО НАПРАВЛЕННОГО РЕБРА. Проверка герметичности считает рёбра НЕнаправленными и
   такого не видит вовсе: у каждого ребра по два треугольника, всё сходится. Между тем сдвиг вершин
   мозаики делает клетки НЕВЫПУКЛЫМИ, и разрез по фиксированной диагонали даёт у такой клетки два
   треугольника противоположного обхода; второй `orient` разворачивает лицом наружу — и оба обходят
   диагональ в одну сторону, а поверхность складывается сама на себя. Нормали там вывернуты, и об это
   спотыкаются иные слайсеры.

   Замер до правки: 78 задвоенных рёбер на Ø99 с рисунком 60 мм, 54 на рисунке 95 мм, ноль на 30 мм —
   зависимость от рисунка и выдала мозаику. Диагональ выбирается теперь по клетке. */
console.log('=== мозаика не складывается сама на себя ===');
{
  const dupes = t => { const m = new Map(), k = v => v.map(c => c.toFixed(4)).join(',');
    for(const T of t) for(let i=0;i<3;i++){ const e = k(T[i]) + '|' + k(T[(i+1)%3]);
      m.set(e, (m.get(e)||0) + 1); }
    let n = 0; for(const c of m.values()) if(c > 1) n++; return n; };
  for(const [shape, w, ring, part] of [['round',60,'1','body'], ['round',95,'1','body'],
                                       ['square',60,'1','body'], ['round',60,'1','ink1'],
                                       ['round',30,'0','body']]){
    logos.length = 0;
    Object.assign(paramState.box, defaultBoxParams(), {csMode:shape, csD:99, csT:3, csRim:0,
      csRingInk:ring, csRingW:3, csRingGap:1.5, csInlay:0.8, csPart:part, csCorner:10});
    logos.push({id:1, face:'-Y', u0:0, v0:0, w:w, h:w, rotation:0, depth:-0.6,
                threshold:0.5, invert:false, heightmap:HM, levels:2});
    const t = buildTrisForShape('box', paramState.box);
    chk(shape + ' ' + w + ' мм / ' + part + ': ни одного задвоенного направленного ребра',
        dupes(t) === 0, {дублей:dupes(t)});
  }
  /* И сам выбор диагонали — на той самой клетке из замера. У невыпуклого четырёхугольника ровно одна
     диагональ внутренняя; знаки площадей по ней обязаны совпасть, по внешней — разойтись. */
  const cr = (a,b,c) => (b[0]-a[0])*(c[1]-a[1]) - (b[1]-a[1])*(c[0]-a[0]);
  const q00 = [2.406,-21.131], q10 = [2.625,-21.175], q11 = [2.600,-20.975], q01 = [2.520,-21.070];
  chk('невыпуклая клетка: по одной диагонали знаки расходятся',
      (cr(q00,q10,q11) > 0) !== (cr(q00,q11,q01) > 0));
  chk('...а по другой совпадают — она и берётся',
      (cr(q10,q11,q01) > 0) === (cr(q10,q01,q00) > 0));
  logos.length = 0;
}
...
console.log('\n=== подстаканник называет слои цвета и ёмкость борта ===');
{
  ...
  /* СЛОИ ЦВЕТА. Карман — первые слои детали, и их число решает, просвечивает цвет или нет. */
  {
    ...
    chk('слоёв цвета — это карман, делённый на высоту слоя',
        Math.abs(g.inlay/layerOf({}) - 4) < 1e-9, +(g.inlay/layerOf({})).toFixed(1));
...
/* КАРМАН, НЕ КРАТНЫЙ СЛОЮ (v25.27.0). Правило стало возможным только когда высота слоя перестала
   быть зашитым числом: смена филамента идёт ПО границе слоя, и дно кармана между границами слайсер
   округлит — цвет выйдет на слой толще или тоньше заказанного. Справка советовала брать кратную
   глубину с незапамятных времён, а проверить это приложение не могло. */
console.log('\n=== подстаканник: карман и высота слоя ===');
{
  const W = (ov) => collectPrintWarnings(Object.assign(defaultBoxParams(), {csMode:'plain'}, ov))
    .filter(x => /не кратен слою/.test(x));
  chk('на умолчаниях карман кратен слою и жалобы нет', W({}).length === 0, W({}));
  chk('  и на других кратных тоже молчит',
      W({csInlay:1.2, printLayerH:0.3}).length === 0 && W({csInlay:0.6, printLayerH:0.2}).length === 0);
  chk('нецелый карман назван вместе с промахом',
      /карман 0\.85 мм не кратен слою 0\.2: это 4\.25 слоя, и дно ляжет на 0\.05 мм мимо границы/
        .test(W({csInlay:0.85}).join(' ')), W({csInlay:0.85}));
  chk('  и предложена ближайшая кратная глубина',
      /Возьмите карман 0\.80 мм — это ровно 4 слоя/.test(W({csInlay:0.85}).join(' ')));
  /* ОДИН И ТОТ ЖЕ КАРМАН ПРИ РАЗНОМ СЛОЕ — то жалоба, то нет: правило смотрит на ПРИНТЕР, а не на
     число миллиметров, и это его смысл. */
  chk('тот же карман при другом слое перестаёт быть виной',
      W({csInlay:0.9, printLayerH:0.2}).length === 1 && W({csInlay:0.9, printLayerH:0.3}).length === 0,
      {'слой 0.2':W({csInlay:0.9, printLayerH:0.2}).length, 'слой 0.3':W({csInlay:0.9, printLayerH:0.3}).length});
}
