// Regression tests for splitting logoResolution into separate solid-box / hollow-container
// controls, and for the tris.push(...bigArray) stack-overflow bug this work turned up in
// buildHollowBox (fixed by switching to concat). Run against the real file, same as
// test_debounce_flow.js / test_hollow_inner_logo.js:
//
...
console.log('\n=== Solid-box resolution and hollow resolution are independent ===');
{
  ...
  logoResolution = 250; logoResolutionHollow = 20;
  const trisSolidHiRes = buildTrisForShape('box', paramState.box);

  logoResolution = 20; // same box/logo, only the SOLID-relevant variable changed
  ...
  logoResolutionHollow = 20;
  const trisHollowA = buildTrisForShape('box', paramState.box);
  logoResolutionHollow = 80;
  const trisHollowB = buildTrisForShape('box', paramState.box);
  check('changing logoResolutionHollow changes the hollow output', trisHollowA.length !== trisHollowB.length, {a: trisHollowA.length, b: trisHollowB.length});

  // The key property this feature is FOR: cranking logoResolution (solid) sky-high must not
  // affect the hollow build at all, and vice versa.
  logoResolution = 300;
  const trisHollowUnaffected = buildTrisForShape('box', paramState.box); // still hollow=true, logoResolutionHollow=80
  check('solid-box resolution does not leak into the hollow build', trisHollowUnaffected.length === trisHollowB.length, {unaffected: trisHollowUnaffected.length, expected: trisHollowB.length});
}

console.log('\n=== Hollow resolution at its UI max (150) stays fast and correct ===');
{
  ...
  for (const l of logos) clampLogoToFace(l);
  logoResolutionHollow = 150;
  const t0 = Date.now();
  const tris = buildTrisForShape('box', paramState.box);
  const ms = Date.now() - t0;
  console.log('    logoResolutionHollow=150, 2 logos incl. inner-bottom: ' + tris.length + ' tris in ' + ms + 'ms');
  check('no NaN at max hollow resolution', !hasNaN(tris));
  check('watertight at max hollow resolution', manifoldCheck(tris,4).watertight, manifoldCheck(tris,4));
  check('completes comfortably fast at the UI max', ms < 1000, ms);
}
