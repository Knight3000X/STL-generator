...
{ // ...and when it does, the part says so. A silently blind cap looks like a bug in the app.
  ...
  /* Строк «сама о себе» у резьбы теперь четыре: числа посадки, числа ХОДОВОЙ резьбы (подъём,
     самоторможение, КПД), сколько пара ДЕРЖИТ и как на эти два числа влияет заполнение. Ни одна не
     жалоба — все они говорят то, чего на экране нет, — и все из счёта исключаются. Начало «резьба»
     без Ø: со стандартным рядом строка начинается с его имени («резьба M8 (ISO 261…)»). */
  const complaints = ov => collectPrintWarnings(setp(ov))
    .filter(s => !/^резьба /.test(s) && !/^подъём витка /.test(s) &&
                 !/^держит /.test(s) && !/^чтобы дать /.test(s) && !/^заполнение бьёт /.test(s));
