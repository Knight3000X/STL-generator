...
{
  reset(); importedPaint.clear();
  // Коробка a×b×c: объём и площадь каждой грани известны точно, поэтому граммы и доли сверяются с
  // ЧИСЛОМ, а не с «примерно». Грани идут парами треугольников: 0-1 это y=0, 2-3 это y=b, 4-5 это z=0,
  // 6-7 это x=a, 8-9 это z=c, 10-11 это x=0.
  const boxTris = (a, b, c) => { const V = [[0,0,0],[a,0,0],[a,0,c],[0,0,c],[0,b,0],[a,b,0],[a,b,c],[0,b,c]];
    const F = [[0,1,2],[0,2,3],[4,7,6],[4,6,5],[0,4,5],[0,5,1],[1,5,6],[1,6,2],[2,6,7],[2,7,3],[3,7,4],[3,4,0]];
    return F.map(f => f.map(i => V[i])); };
  const cube = v => boxTris(v, v, v);
  ...
  /* РАСКРАШЕННАЯ ДЕТАЛЬ. Своего объёма у покраски нет — номер филамента стоит у ТРЕУГОЛЬНИКА, а не у
     куска тела, — поэтому объём делится по ПЛОЩАДИ.

     Коробка нарочно НЕ кубическая: 10×10×20 это 2 см³ и 1000 мм² поверхности, из них торец 10×10 — сто,
     то есть ровно десятая. Считай программа доли по ЧИСЛУ треугольников, вышло бы 2 из 12, то есть
     шестая, и на кубе эти два ответа совпали бы — разошлись они только здесь. */
  reset(); importedPaint.clear();
  const bx = boxTris(10, 10, 20);
  const ink = bx.map((t, i) => (i === 4 || i === 5) ? 2 : 1);   // торец z=0, площадь 100 из 1000
  addImportedPart(bx, 'Крашеный', false, '#101010', null, { ink, palette: ['#101010', '#00AA00'] });
  gr = assemblyGrams();
  check('раскрашенная деталь просит два слота', gr.cc.length === 2, gr.cc);
  check('покраске досталась её доля ПЛОЩАДИ, а не доля счёта',
        near(gr.cc[1], 2*0.1, 1e-9), {дали: gr.cc[1], поПлощади: 0.2, поСчёту: 2/6});
  check('остальное — базовому цвету', near(gr.cc[0], 2*0.9, 1e-9), gr.cc[0]);
  check('и объём детали не потерялся и не удвоился', near(gr.total, 2, 1e-9), gr.total);

  /* РАСКЛАДКА, НЕ ПЕРЕЖИВШАЯ ПЕРЕСБОРКУ СЕТКИ. Соответствие «треугольник ↔ филамент» сквозь пересборку
     не протаскивается — оно ПРОВЕРЯЕТСЯ по длине. Здесь раскладка заведена на четырёх треугольниках, а
     сетка потом стала двенадцатью: применить её к чужой сетке значит покрасить наугад, поэтому деталь
     обязана уйти в свой базовый цвет целиком. */
  reset(); importedPaint.clear();
  const tetra = [[[0,0,0],[0,10,0],[10,0,0]], [[0,0,0],[10,0,0],[0,0,10]],
                 [[0,0,0],[0,0,10],[0,10,0]], [[10,0,0],[0,10,0],[0,0,10]]];
  addImportedPart(tetra, 'Пересобранный', false, '#101010', null,
                  { ink: [1, 2, 1, 1], palette: ['#101010', '#00AA00'] });
  models[models.length-1].rawTris = cube(10);          // сетку пересобрали: двенадцать вместо четырёх
  gr = assemblyGrams();
  check('раскладка чужой длины не применяется', gr.cc.length === 1 && near(gr.cc[0], 1, 1e-9), gr.cc);
