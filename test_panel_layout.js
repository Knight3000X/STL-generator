...
/* НИЗ ПАНЕЛИ УХОДИЛ ПОД СИСТЕМНУЮ ПОЛОСУ. В `<meta viewport>` стоит `viewport-fit=cover`: страница
   СОГЛАСИЛАСЬ рисоваться под вырезом и под полосой жестов — ради того, чтобы синька уходила под самую
   кромку, — а взамен обязана сама отступить от них на `env(safe-area-inset-*)`. Ни одного такого отступа в
   файле не было ни разу: панель кончалась ровно на физической кромке экрана (замерено в headless Chromium:
   `panelBottom` совпадал с `innerHeight` до пикселя), и последние 24…48 px её содержимого приходились на
   полосу жестов. Со стороны это «не пролистывается до конца»: прокрутка доходит, а конец лежит под полосой.

   Прежняя правка того же симптома (v16.5.3, `dvh` вместо `vh`) была про ДРУГУЮ причину — про высоту окна,
   пока браузер ещё показывает свои панели, — и потому не помогла в установленном приложении, где панелей
   браузера нет вовсе. Полоса жестов лежит ВНУТРИ окна, и никакая единица высоты о ней не знает.

   Проверяется по тому же перечню прокручиваемых контейнеров, что и жёлоб: раз контейнер доходит до кромки
   и прокручивается, он обязан отвечать за отступ сам — и новый, добавленный завтра, тоже. */
console.log('=== низ уходит из-под системной полосы ===');
{
  const meta = /<meta\s+name="viewport"[^>]*>/i.exec(HTML);
  chk('в meta есть viewport-fit=cover', !!meta && /viewport-fit\s*=\s*cover/.test(meta[0]), meta && meta[0]);
  chk('и раз так, отступы safe-area в стилях есть', /env\(\s*safe-area-inset-/.test(CSS),
      (CSS.match(/env\(\s*safe-area-inset-[a-z]+/g) || []).length + ' шт.');
  ...
  for (const r of scrollers){
    /* Отступ снизу задан либо сокращением `padding`, либо отдельным `padding-bottom` — годится любое,
       лишь бы в нём был системный отступ. Разбирать сокращение на составляющие нельзя: там `calc()`. */
    const pad = [decl(r.body, 'padding'), decl(r.body, 'padding-bottom')].filter(Boolean).join(' ');
    chk('«' + r.sel + '» отступает снизу на системную полосу',
        /env\(\s*safe-area-inset-bottom/.test(pad), pad || '(отступа нет вовсе)');
  }
  /* И запас сверх системного отступа: там, где система сообщает ноль (кнопочная навигация со своей
     полосой), последняя строка всё равно не должна липнуть к кромке. Замерено в headless, где env = 0:
     до правки между последней строкой и низом панели оставалось 32 px, теперь 44. */
  { const mob = allWithSel('#panel').map(r => decl(r.body, 'padding-bottom')).filter(Boolean);
    chk('и берёт запас поверх него', mob.some(v => /4[0-9]px|5[0-9]px/.test(v)), mob); }
  /* Кнопки поверх превью — тот же счёт с другой стороны: вырез камеры лежит сверху. */
  for (const id of ['#btn-reset-view', '#btn-move-mode']){
    const r = allWithSel(id)[0];
    chk(id + ' отступает от выреза сверху',
        !!r && /env\(\s*safe-area-inset-top/.test(decl(r.body, 'top') || ''), r && decl(r.body, 'top'));
  }
}
