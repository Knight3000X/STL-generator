...
console.log('=== ПЕРЕПИСЬ МОЛЧУНОВ (10.4): список может только сокращаться ===');
{
  ...
  /* ОСТАЛОСЬ ДВОЕ, и оба — по делу. Кость молчит, пока на гранях нет цифр (объяснено ниже), а куб это
     куб: ширина, высота и глубина стоят на панели, и числа, которого не видно, у него нет. Разбор 10.4
     на этом и кончается — не потому, что список надоел, а потому что он кончился. */
  const SILENT = ['die', 'box'];
