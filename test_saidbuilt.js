   ...
   бы выдумать связь. Такие выражения перепись пропускает НАРОЧНО, и проверка на это стоит ниже. */
{
  ...
  for (const m of src.matchAll(/(^|[^=!<>+\-*/%&|^])=(?![=>])\s*|\breturn\s+/gm)){
  ...
  chk('машинка переписи что-то видит', Object.keys(hits).length > 800, Object.keys(hits).length);
  ...
  for (const m of fake.matchAll(/(^|[^=!<>+\-*/%&|^])=(?![=>])\s*|\breturn\s+/gm)){
    const i = m.index + m[0].length, k = keyOf(fake.slice(i, exprEnd(fake, i)));
    const c = sizeExprKind(k);
    if (c && c !== 'зажим') (seen[k] = seen[k] || []).push(i);
  }
  const found = Object.entries(seen).filter(([, v]) => v.length === 2).map(([k]) => k).sort();
  chk('перепись поймала бы все три подложенные копии', found.length === 3, found);
