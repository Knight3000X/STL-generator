   ...
   одну деталь, печатая другую. Число закреплено на НУЛЕ и может только остаться нулём. */
{
  ...
  /* ТРЕБОВАНИЕ «СПЕЦИФИКАЦИЯ + ПОСТРОИТЕЛЬ» СНЯТО (v25.40.0), и это была вторая её слепота. Оно
     разумно по происхождению — именно так дали о себе знать две настоящие ошибки, — но копия между
     ДВУМЯ ПОСТРОИТЕЛЯМИ (корпус и крышка футляра, червяк и его огибающая, две коробки с
     перегородками) или между построителем и ПРЕДУПРЕЖДЕНИЯМИ для неё не существовала вовсе. Замер без
     этого требования дал 26 выражений в разных функциях; все разобраны в v25.40.0.
     Достаточно РАЗНЫХ ФУНКЦИЙ: одно и то же выражение, написанное дважды в одной функции, — это её
     собственное дело, а вот два хозяина у одного числа — это два места, где его можно поправить. */
  /* САМО ПРАВИЛО — ОТДЕЛЬНЫМ ИМЕНЕМ, и проверка ниже спрашивает его же. Сузь его обратно до «одно
     место — спецификация, другое — построитель», и перепись снова перестанет видеть копию между двумя
     построителями; без этой проверки такое сужение прошло бы молча. */
  const dupRule = names => names.length > 1;
  const dup = [];
  for (const [k, v] of Object.entries(hits)){
    if (v.length < 2) continue;
    const names = [...new Set(v.map(owner))];
    if (dupRule(names)) dup.push(k.slice(0, 80) + '  ← ' + names.join(' | '));
  }
  ...
  const DUP_MAX = 0;   // v25.35.0: 23 пары «спец + построитель» → 0; v25.39.0: +28 по пробелам; v25.40.0: +26 вне пары
