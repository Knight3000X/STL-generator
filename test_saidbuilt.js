   ...
   бы выдумать связь. Такие выражения перепись пропускает НАРОЧНО, и проверка на это стоит ниже. */
{
  ...
  const KNOB = /(?<![A-Za-z0-9_$])p\.\w+/;        // именно ручка, а не хвост чужого имени (`inp.value`)
  ...
  for (const m of src.matchAll(/Math\.(?:max|min)\(/g)){
    const e = closeOf(src, m.index + m[0].length - 1); if (e < 0) continue;
    const k = keyOf(src.slice(m.index, e + 1));
  ...
  const ENTRIES = [/(^|[^=!<>+\-*/%&|^])=(?![=>])\s*|\breturn\s+/gm, /[(,]\s*/g];
  for (const re of ENTRIES) for (const m of src.matchAll(re)){
    const i = m.index + m[0].length, raw = src.slice(i, exprEnd(src, i));
  ...
  const fake =
    ...
    "  const c = tstNum(p.zzzAng, 40, 85, 80); return a+b+c; }\n" +
    "function buildFoo(p){ const a=Math.max(3,p.zzz||7);\n  const b=p.zzzMode||'none';\n" +
    "  const c=tstNum(p.zzzAng,40,85,80); return a+b+c; }\n";
  const seen = {};
  for (const m of fake.matchAll(/Math\.(?:max|min)\(/g)){
    const e = closeOf(fake, m.index + m[0].length - 1); if (e < 0) continue;
    const k = keyOf(fake.slice(m.index, e + 1));
    if (sizeExprKind(k)) (seen[k] = seen[k] || []).push(m.index);
  }
  for (const re of ENTRIES) for (const m of fake.matchAll(re)){
    const i = m.index + m[0].length, k = keyOf(fake.slice(i, exprEnd(fake, i)));
    const c = sizeExprKind(k);
    if (c && c !== 'зажим') (seen[k] = seen[k] || []).push(i);
  }
  ...
  /* ЧЕГО ПЕРЕПИСЬ НЕ УВИДИТ И ПОСЛЕ ЭТОЙ СБОРКИ, СКАЗАНО ЗДЕСЬ ЖЕ. Ключ чувствителен к ИМЕНАМ
     переменных: `BOX_FACE_DIMS[H.face] ? … : …` и `BOX_FACE_DIMS[hole.face] ? … : …` для неё разные
     выражения, хотя это одна формула. Сведение имён к общему виду нашло пять таких пар, из них две
     настоящие (эта и длина резьбы накидной гайки — там третья копия звала шаг `P2` вместо `P`), а
     три ложные: `p.pbPart || 'none'` и `p.pbPart || 'tray'` — разные вопросы, а не копия. Поэтому
     приём в перепись НЕ взят, обе настоящие разобраны руками, а вместо переписи здесь стоит счёт:
     обращение к таблице размеров грани мимо общего правила должно оставаться редким. */
