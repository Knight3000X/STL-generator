...
console.log('=== подставка говорит, когда упадёт ===');
{
  ...
  // 1. МАССА И ЦЕНТР — ИЗ СЕТКИ, И ЦЕНТР НАСТОЯЩИЙ
  {
    ...
    cz /= V; V = Math.abs(V);
    chk('масса взята из объёма построенной сетки',
        Math.abs(s.mass - V/1000*matDensity(P_({}))) < 1e-6, {спец:+s.mass.toFixed(2), сетка:+(V/1000*matDensity(P_({}))).toFixed(2)});
    chk('  и центр тяжести — настоящий, а не середина габарита',
        Math.abs(s.cog[2] - cz) < 1e-6, {спец:+s.cog[2].toFixed(3), сетка:+cz.toFixed(3)});
