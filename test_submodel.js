...
console.log('=== у каждого семейства ровно одна головная форма ===');
{
  ...
  /* Число названо, а не выведено, и это нарочно: семейство — редкое событие, и появление следующего
     должно уронить батарею, а не пройти молча. Список рядом с числом, чтобы падение сразу говорило,
     кто прибавился. */
  chk('семейств девять', fams.size === 9, [...fams]);
  chk('и это они',
      [...fams].sort().join() === 'clMode,frMode,gearMode,mntMode,pipMode,sealMode,spMode,threadMode,tstMode',
      [...fams].sort());
  const found = {};
