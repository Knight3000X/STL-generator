...
console.log('=== резьба: правило корня и горла живёт в одном месте ===');
{
  ...
  /* ПРИМЕЧАНИЯ ВЫРЕЗАНЫ (v25.43.0). Счёт «написано ровно один раз» обязан считать код: приведи
     кто-нибудь ту же формулу В ПОЯСНЕНИИ — и проверка объявила бы копию там, где её нет. Ниже это
     уже обходили руками, требуя оператор перед числом; теперь обход не нужен, а требование
     оставлено: оно и само по себе верно. Помощник — общий, из `stub_preamble.js`. */
  const app = __stripComments(src.split('<script>').slice(2).join('<script>'));   // 1-й блок — Three.js
