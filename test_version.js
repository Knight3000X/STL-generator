...
console.log('=== приложение показывает то, что посчиталось ===');
{
  ...
  /* `version.json` — третий носитель того же номера, и он самый важный из трёх: это единственное, по чему
     установленное приложение узнаёт, что его кэш устарел (оболочка на 2.5 МБ отдаётся из кэша, качать её
     ради проверки версии нельзя). Разойдись он с APP_VERSION — и приложение либо будет вечно предлагать
     обновление, которое ничего не меняет, либо промолчит о настоящем. */
  const vj = JSON.parse(fs.readFileSync('version.json', 'utf8'));
  chk('у version.json есть версия', typeof vj.version === 'string', vj);
  chk('и она ровно та же, что у приложения', vj.version === APP_VERSION, [vj.version, APP_VERSION]);
  chk('version.json лежит в оболочке воркера — иначе офлайн он не откроется',
      /'version\.json'/.test(sw));
  chk('и НЕ берётся из кэша: иначе он вечно подтверждал бы сам себя',
      /version\.json[\s\S]{0,400}?fetch\(freshRequest/.test(sw));
}
...
console.log('=== реестр не разошёлся с историей репозитория ===');
{
  ...
  /* Коммиты без строки, с причиной — см. «Чего реестр НЕ обещает». Шесть из них — разбор присланных
     образцов: оснастка и заметки, приложения не трогают вовсе, а значит и выпуском не являются.
     Последний, `611e975`, другого рода: промежуточный снимок работы над v22.10.0, сделанный ради
     сохранности (контейнер сессии откатывал дерево шесть раз, и origin — единственное, что откаты
     переживает). Выпуском он не был — до конца работы ничего не работало, — и всё его содержимое вошло
     в следующую строку. Впредь такой снимок дописывается в себя же через `--amend`, а не оставляет
     второй коммит; здесь это было уже нельзя — снимок успел уйти в main в составе истории выпуска.
     `7c3f8f3` и `4e8a830` — третьего рода: хуки сессии и разбор отката контейнера. Приложения они не
  ...
  if (!hist) {
    ...
    /* И СПИСОК ИСКЛЮЧЕНИЙ НЕ ЗАКРЫВАЕТ РОТ БЕЗ ОБЪЯСНЕНИЯ. Дописать сюда хеш — самый простой способ
       заставить эту проверку замолчать, и без сторожа он ничем не отличается от исправления. Поэтому
       каждое исключение обязано быть НАЗВАНО в VERSION.md, в разделе «Чего реестр НЕ обещает», со своей
       причиной: молчание тогда стоит абзаца текста, а не одной строки в массиве. */
    const unexplained = EXCEPT.filter(h => LEDGER.indexOf(h) < 0);
    chk('каждое исключение объяснено в VERSION.md', unexplained.length === 0, unexplained);
    // Исключения — это не выпуски: хеш не может стоять и в списке, и строкой реестра.
    chk('исключение не притворяется строкой реестра',
        EXCEPT.every(h => !ls.has(h)), EXCEPT.filter(h => ls.has(h)));
