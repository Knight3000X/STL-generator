...
{ /* Cheap enough to keep in the interactive path: the field is computed over ONE tooth pitch and copied.
     ...
     восемь файлов на восьми ядрах, и на загруженной машине этот же самый расчёт «не укладывается в
     ...
     Процессорное время убрало зависимость от ЗАГРУЗКИ машины, но не от самой машины, и абсолютный порог
     пришлось поднимать дважды: 3600 → 4200 → и снова мало. На третий раз стало видно, что чинится не то.
     БЕЗ ЕДИНОЙ ПРАВКИ в этом коде, на одном контейнере, за несколько минут тот же расчёт дал 3195, 3394,
     3415, 4306, 4551 и 4968 мс процессорного времени — полтора раза разницы. Никакой абсолютный порог
     такого не переживёт: он сообщает о настроении контейнера, а не о стоимости кода.

     Поэтому порог теперь ОТНОСИТЕЛЬНЫЙ. Рядом считается короткий эталон из той же арифметики, что и в
     `inside` — гипотенуза, арктангенс, ветвление, — и стоимость нарезки выражается в его единицах. Обе
     величины плывут вместе, и отношение держится там, где абсолютное время скачет: те же шесть прогонов
     дали 13.9…16.6 при разбросе самого времени в полтора раза.

     Чего этот порог НЕ делает, лучше сказать прямо. Отвергнутый в v16.5.6 шаг марша 0.03 стоил бы примерно
     на 28 % дороже — то есть 17.8…21.2 в тех же единицах, — и с порогом 20 это уже не разделяется надёжно.
     Проверка осталась грубой страховкой от блокирующего просчёта (вдвое и хуже), а не тонким инструментом;
     тонким она и не была с тех пор, как разброс железа перекрыл разницу между хорошим и плохим шагом. */
  // Эталон: та же арифметика, что и в `inside`, на фиксированном числе итераций. Минимум из трёх — минимум
  // и есть честная оценка стоимости, разброс сверху даёт сборщик мусора и планировщик.
  let calMs = Infinity;
  for(let k=0;k<3;k++){ const c0=process.cpuUsage(); let acc=0;
    for(let i=0;i<3e6;i++){ const x=i*0.001, y=Math.hypot(x,1.5), a=Math.atan2(y,x+1); acc += (a<0.5?y:-y); }
    const c=process.cpuUsage(c0); calMs=Math.min(calMs,(c.user+c.system)/1000);
    if(!isFinite(acc)) throw new Error('эталон посчитался в NaN'); }
  base({gearWheelRim:'envelope', gearTeeth:20, gearThick:8});                        // прогрев JIT
  let ms = Infinity;
  for(let i=0;i<3;i++){ const c0=process.cpuUsage(); base({gearWheelRim:'envelope', gearTeeth:60, gearThick:16});
    const c=process.cpuUsage(c0); ms = Math.min(ms, (c.user+c.system)/1000); }
  const units = ms/Math.max(1, calMs);
  chk('нарезка укладывается в бюджет пересчёта', units < 20, {units:+units.toFixed(1), ms:+ms.toFixed(0), cal:+calMs.toFixed(0)});
}
