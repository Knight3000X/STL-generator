...
{ /* Cheap enough to keep in the interactive path: the field is computed over ONE tooth pitch and copied.
     ...
     Процессорное время убрало зависимость от ЗАГРУЗКИ машины, но не от самой машины: столько-то
     миллисекунд процессорного — это по-прежнему такт конкретного железа. Порог 3600 был перемерян там,
     где нынешний шаг марша (dr = 0,045) обходился в 3,1–3,2 с, а отвергнутый в v16.5.6 шаг 0,03 — в
     4,0–4,1 с. На контейнере помедленнее тот же расчёт БЕЗ ЕДИНОЙ ПРАВКИ в этом коде дал 3716 / 4223 /
     3647 мс, то есть 3600 оказалось ниже шумового пола, и тест начал сообщать о железе, а не о коде.

     Отсюда две поправки. Берётся МИНИМУМ из трёх прогонов после прогрева — минимум и есть честная оценка
     стоимости, а разброс сверху даёт сборщик мусора и планировщик. И порог 4200 вместо 3600: минимум
     хорошего кода здесь 3647–3686, минимум отвергнутого шага 0,03 был бы примерно вчетверть больше
     (~4670 по тому же отношению 1,28). 4200 лежит между ними — с запасом в 15 % сверху от хорошего и
     11 % снизу от плохого. Относительной эту проверку сделать не вышло: единственный дешёвый эталон
     рядом — прямой обод, а он стоит 50–130 мс и шумит так, что отношение прыгает с 61 до 70. */
  ...
  chk('нарезка укладывается в бюджет пересчёта', ms < 4200, {ms:+ms.toFixed(0)});
}
