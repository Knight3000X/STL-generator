...
loadParts(process.argv[2]).then(parts => {
  const p = parts[+(process.argv[3]||0)], step = +(process.argv[4]||0.3);
  const m = heightMap(p.tris, step), plate = plateTop(m);
  console.log(p.name+'   верх плиты ≈ '+plate.toFixed(2)+'   шаг сетки '+step);
  const keep = b => b.area > 1.5;
  const up    = blobs(m, v => v > plate + 0.6).filter(keep);
  /* Сквозное отверстие всегда ВНУТРИ детали. Область пустоты, касающаяся края карты, — это не дыра, а
     полклетки за габаритом: сетка почти никогда не делит габарит нацело, и такая «дыра» в 0.3 мм шириной
     во всю длину плиты появлялась на каждом разборе. */
  const holes = blobs(m, v => v < -1e8).filter(b => b.area > 1.0 && !b.edge);
  const down  = blobs(m, v => v > -1e8 && v < plate - 0.4).filter(keep);
  const pos = b => 'x='+b.cx.toFixed(1).padStart(6)+' y='+b.cy.toFixed(1).padStart(6)+
                   '  '+(b.w.toFixed(1)+'×'+b.d.toFixed(1)).padStart(12);
  console.log('\n-- ВЫСТУПЫ ('+up.length+')');
  for(const b of up.sort((a,c)=>a.cy-c.cy||a.cx-c.cx))
    console.log('   '+pos(b)+'  высота='+(b.hi-plate).toFixed(2).padStart(6)+'  площадь='+b.area.toFixed(0));
  console.log('\n-- СКВОЗНЫЕ ОТВЕРСТИЯ ('+holes.length+')');
  for(const b of holes.sort((a,c)=>a.cy-c.cy||a.cx-c.cx)) console.log('   '+pos(b));
  console.log('\n-- КАРМАНЫ ('+down.length+')');
  for(const b of down.sort((a,c)=>a.cy-c.cy||a.cx-c.cx))
    console.log('   '+pos(b)+'  глубина='+(plate-b.lo).toFixed(2));
