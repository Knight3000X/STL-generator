...
/* Профиль верхней поверхности вдоль прямой. Сечение честнее карты сверху: по нему видно, ДУГА кромка или
   срез, и на какой длине она набирает высоту, — а именно это и надо, чтобы повторить станцию, а не
   угадать её. Печатаются только ИЗМЕНЕНИЯ высоты, иначе вывод — километр одинаковых строк. */
loadParts(process.argv[2]).then(parts => {
  const p = parts[+process.argv[3]], axis = process.argv[4];
  const fixed = +process.argv[5], a0 = +process.argv[6], a1 = +process.argv[7], step = +(process.argv[8]||0.25);
  const oth = axis === 'x' ? 1 : 0;
  const sub = p.tris.filter(T => Math.min(T[0][oth],T[1][oth],T[2][oth]) <= fixed &&
                                 Math.max(T[0][oth],T[1][oth],T[2][oth]) >= fixed);
  console.log(p.name+'   '+(axis==='x'?'по X при Y=':'по Y при X=')+fixed+
              '   от '+a0+' до '+a1+' шагом '+step+'   (Z — вверх)');
  let prev = null;
  for(let u=a0; u<=a1+1e-9; u+=step){
    const x = axis==='x'?u:fixed, y = axis==='x'?fixed:u; let h=-1e9;
    for(const T of sub){ const A=T[0],B=T[1],C=T[2];
      const ar=(B[0]-A[0])*(C[1]-A[1])-(B[1]-A[1])*(C[0]-A[0]); if(Math.abs(ar)<1e-12) continue;
      const w1=((B[0]-x)*(C[1]-y)-(B[1]-y)*(C[0]-x))/ar,
            w2=((C[0]-x)*(A[1]-y)-(C[1]-y)*(A[0]-x))/ar, w3=1-w1-w2;
      if(w1<-1e-9||w2<-1e-9||w3<-1e-9) continue;
      const z=w1*A[2]+w2*B[2]+w3*C[2]; if(z>h) h=z; }
    const s = h < -1e8 ? 'ПУСТО' : h.toFixed(2);
    if(s !== prev){ console.log(u.toFixed(2).padStart(9)+'   '+s); prev = s; }
  }
